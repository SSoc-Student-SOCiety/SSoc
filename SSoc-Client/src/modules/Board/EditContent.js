...
const EditContent = (props) => {
  ...
  const [isChecked, setIsChecked] = useState(content.nickname == '익명' ? true : false)
    ...
    try {
      ...
      if (data != null) {
        if (data.dataHeader.successCode == 0) {
          Alert.alert('게시글이 수정되었습니다.', navigation.goBack(), props.setReload(!props.reload))
        } else {
          Alert.alert(data.dataHeader.resultMessage, props.setEditContent(false))
        }
        return
      }
      Alert.alert('게시글 수정에 실패했습니다.', '알 수 없는 에러', props.setEditContent(false))
    } catch (e) {
      console.log(e)
    }
  }
        ...
        <TouchableWithoutFeedback>
          <View style={{ flexDirection: 'column', width: '90%', backgroundColor: Color.WHITE, borderColor: Color.BLUE, borderWidth: 1, borderRadius: 20 }}>
            <View style={{ backgroundColor: Color.BLUE, width: '100%', height: '12%', borderTopRightRadius: 15, borderTopLeftRadius: 15, alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontSize: 20, color: Color.WHITE, fontWeight: 'bold' }}>게시글 수정</Text>
            </View>
